...
<form action="{{ route('vouchers.index') }}" method="GET" class="mb-4">
  ...
  @endif
      <a href="{{route('vouchers.create')}}"><button class="btn btn-light" data-bs-toggle="modal">Thêm Voucher Mới</button></a>
    </div>
                  ...
                  <tbody>
                    ...
                    @foreach($voucher_arr as $voucher)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('vouchers.edit', $voucher->id_mgg) }}">
                                    <button class="btn btn-sm btn-primary">Chỉnh Sửa</button>
                                </a>

                                <form action="{{ route('vouchers.destroy', $voucher->id_mgg) }}" method="POST" style="display:inline;">
