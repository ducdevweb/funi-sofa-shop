...
class AdminDanhMuc extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('hinh')) {
            ...
            $imagePath = $file->store('danhmuc_img', 'public'); 
        
            $them->hinh_bl = env('APP_URL').'/storage/' . $imagePath;
        }
    ...
    public function update(Request $request, $id_loaisp = 0)
    {
        ...
        if ($request->hasFile('hinh')) {
            ...
            $sua->hinh_bl = env('APP_URL').'/storage/' . $imagePath;
        }
        else {
